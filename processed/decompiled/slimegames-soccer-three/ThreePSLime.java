...
public class ThreePSLime extends Applet implements Runnable {
   private int nWidth;
   private int nHeight;
   private int p1Score;
   private int p2Score;
   ...
   private void DrawSlimers() {
      ...
      this.screen.setColor(Color.blue);
      this.screen.fillRect(i, l, k1, j2);
      i = this.p3OldX * this.nWidth / 1000 - k1 / 2;
      l = 7 * this.nHeight / 10 - this.p3OldY * this.nHeight / 1000;
      this.screen.setColor(Color.blue);
      this.screen.fillRect(i, l, k1, j2);
      if(!this.fEndGame) {
         this.MoveBall();
      ...
      i = this.p3X * this.nWidth / 1000 - k1 / 2;
      l = 7 * this.nHeight / 10 - this.p3Y * this.nHeight / 1000;
      this.screen.setColor(!this.fSuperSlime?this.slimaryCols[this.p2Col]:this.slimaryCols[this.frenzyCol = (this.frenzyCol + 1) % this.slimaryCols.length]);
      this.screen.fillArc(i, l, k1, 2 * j2, 0, 180);
      ...
      if(this.p2Score > this.p1Score + 2) {
         i2 = this.nWidth / 20;
         l2 = this.nHeight / 20;
         k = this.p3X * this.nWidth / 1000 - i2;
         j1 = 7 * this.nHeight / 10 - (this.p3Y - 40) * this.nHeight / 1000;
         k5 = 0;
   ...
   public void paint(Graphics g) {
      ...
      if(!this.fInPlay) {
         ...
         this.screen.setColor(Color.white);
         if(this.fSuperSlime) {
            this.screen.drawString("Super 3P Soccer Slime!", this.nWidth / 2 - fontmetrics.stringWidth("Super Soccer Slime!") / 2, this.nHeight / 2 - fontmetrics.getHeight());
         } else {
            this.screen.drawString("3P Soccer Slime!", this.nWidth / 2 - fontmetrics.stringWidth("Soccer Slime!") / 2, this.nHeight / 2 - fontmetrics.getHeight());
         }
   ...
   private void MoveBall() {
      ...
      if(!this.fEndGame) {
         ...
         if(i2 > 0 && j2 < 15625 && j2 > 25) {
            i1 = (int)Math.sqrt((double)j2);
            k1 = (l1 * k2 + i2 * l2) / i1;
            this.ballX = this.p3X + l1 * 63 / i1;
            this.ballY = this.p3Y + i2 * 125 / i1;
            if(k1 <= 0) {
               if(!this.fP3Sticky) {
                  this.ballVX += (this.p3XV - 2 * l1 * k1 / i1) * 7 / 10;
                  this.ballVY += this.p3YV - 2 * i2 * k1 / i1;
               } else {
                  this.ballVX = 0;
                  this.ballVY = 0;
               }
   ...
   private void method_rn_ThreePSLime_p_1(String s) {
