...
public class WaterPoloSlime extends Applet implements Runnable {
   private int nWidth;
   private int nHeight;
   private int p1Score;
   private int p2Score;
   ...
   private int p1Col;
   private int p2Col;
   private int splashStage = 1;
   private int oldSplashX = 0;
   private int oldSplashY = 0;
   private int oldSplashXV = 0;
   private int part1X = 110;
   private int part1Y = 110;
   private int part1Xold = 0;
   private int part1Yold = 0;
   private int part2Xold = 0;
   private int part2Yold = 0;
   private int part3Xold = 0;
   private int part3Yold = 0;
   private int part4Xold = 0;
   private int part4Yold = 0;
   private int part5Xold = 0;
   private int part5Yold = 0;
   private int part6Xold = 0;
   private int part6Yold = 0;
   private int part2X = 110;
   private int part2Y = 110;
   private int part3X = 110;
   private int part3Y = 110;
   private int part4X = 0;
   private int part4Y = 0;
   private int part5X = 0;
   private int part5Y = 0;
   private int part6X = 0;
   private int part6Y = 0;
   private int part7X = 0;
   private int part7Y = 0;
   private int part8X = 0;
   private int part8Y = 0;
   int splash1 = 0;
   int splash2 = 0;
   int splash3 = 0;
   int p1XVar = 0;
   ...
   int ballXVar = 0;
   int ballYVar = 0;
   int NUM_PARTICLES_HIT = 30;
   int[] particle_x_HIT;
   int[] particle_y_HIT;
   int[] particle_yv_HIT;
   int[] particle_xv_HIT;
   int NUM_PARTICLES_HIT2;
   int[] particle_x_HIT2;
   int[] particle_y_HIT2;
   int[] particle_yv_HIT2;
   int[] particle_xv_HIT2;
   int NUM_PARTICLES_HIT3;
   int[] particle_x_HIT3;
   int[] particle_y_HIT3;
   int[] particle_yv_HIT3;
   int[] particle_xv_HIT3;
   ...
   private void DrawSlimers() {
      ...
      this.screen.fillRect(0, 4 * this.nHeight / 5 - 20, this.nWidth / 1, 20);
   ...
   public void paint(Graphics g) {
      ...
      this.screen.setColor(Color.white);
      this.screen.fillRect(0, 4 * this.nHeight / 5, this.nWidth, this.nHeight / 6);
      this.screen.setColor(Color.white);
      this.screen.drawLine(0, 4 * this.nHeight / 5 - 20, this.nWidth, this.nHeight / 6 + 170);
      this.screen.setColor(Color.white);
      this.drawScores();
      if(!this.fInPlay) {
         this.DrawSlimers();
      ...
      if(!this.fInPlay) {
         ...
         this.screen.setColor(Color.white);
         if(this.fSuperSlime) {
            this.screen.drawString("Super Water Polo Slime!", this.nWidth / 2 - fontmetrics.stringWidth("Super Soccer Slime!") / 2, this.nHeight / 2 - fontmetrics.getHeight());
         } else {
            this.screen.drawString("Water Polo Slime!", this.nWidth / 2 - fontmetrics.stringWidth("Soccer Slime!") / 2, this.nHeight / 2 - fontmetrics.getHeight());
         }
   ...
   public WaterPoloSlime() {
      this.particle_x_HIT = new int[this.NUM_PARTICLES_HIT];
      this.particle_y_HIT = new int[this.NUM_PARTICLES_HIT];
      this.particle_yv_HIT = new int[this.NUM_PARTICLES_HIT];
      this.particle_xv_HIT = new int[this.NUM_PARTICLES_HIT];
      this.NUM_PARTICLES_HIT2 = 30;
      this.particle_x_HIT2 = new int[this.NUM_PARTICLES_HIT2];
      this.particle_y_HIT2 = new int[this.NUM_PARTICLES_HIT2];
      this.particle_yv_HIT2 = new int[this.NUM_PARTICLES_HIT2];
      this.particle_xv_HIT2 = new int[this.NUM_PARTICLES_HIT2];
      this.NUM_PARTICLES_HIT3 = 30;
      this.particle_x_HIT3 = new int[this.NUM_PARTICLES_HIT3];
      this.particle_y_HIT3 = new int[this.NUM_PARTICLES_HIT3];
      this.particle_yv_HIT3 = new int[this.NUM_PARTICLES_HIT3];
      this.particle_xv_HIT3 = new int[this.NUM_PARTICLES_HIT3];
      this.slimeColText = new String[]{"Argentina", "Belgium", "Australia", "Iceland", "Cameroon", "P.R. of China", "Costa Rica", "Croatia", "Denmark", "Eucador", "Mexico", "France", "USA", "Italy", "Japan", "Russia", "Paraguay", "Poland", "Portugal", "Ireland", "Saudi Arabia", "Senegal", "Slovenia", "Spain", "Seth Efrica", "South Corea", "Sveden", "Tunisia", "Turkey", "Uruguay", "Brazil", "England", "Germany", "Night Elves"};
   ...
   private void MoveBall() {
      ...
      if(!this.fEndGame) {
         ...
         if(this.ballY < 84) {
            this.ballY = 84;
            this.ballVY = -this.ballVY * 3 / 10;
            this.ballVX = this.ballVX * 3 / 10;
         }
      }
   ...
   private void DrawGoals() {
      ...
      this.screen.fillRect(0, 4 * this.nHeight / 5 - 21, this.nWidth / 1, 22);
      this.screen.setColor(new Color(10, 0, 210));
      this.screen.fillRect(0, 4 * this.nHeight / 5 - 20, this.nWidth / 1, 20);
      this.screen.setColor(Color.white);
      ...
      p1TickX = (60 - this.p1TouchingGoal) * this.nWidth / 120;
      this.screen.setColor(this.secondaryCols[this.p1Col]);
      ...
      int p2TickX = this.nWidth - (60 - this.p2TouchingGoal) * this.nWidth / 120;
      this.screen.setColor(this.secondaryCols[this.p2Col]);
   ...
   private void updateParticlesHit() {
      for(int i = 0; i < this.NUM_PARTICLES_HIT; ++i) {
         int incrY = (int)(Math.random() * 2.0D + 1.0D);
         int incrX = (int)(Math.random() * (double)this.ballVX + 5.0D);
         int randY = (int)(Math.random() * (double)this.ballVY + 1.0D);
   ...
   private void drawParticlesHit() {
      for(int i = 0; i < this.NUM_PARTICLES_HIT; ++i) {
         int rand = (int)(Math.random() * 3.0D);
         if(rand == 1) {
   ...
   private void updateParticlesHit2() {
      for(int i = 0; i < this.NUM_PARTICLES_HIT2; ++i) {
         int incrY = (int)(Math.random() * 5.0D + 1.0D);
         int incrX = (int)(Math.random() * (double)this.p1XV + 1.0D);
         int randY = (int)(Math.random() * (double)this.p1YV / 2.0D + 1.0D);
   ...
   private void drawParticlesHit2() {
      for(int i = 0; i < this.NUM_PARTICLES_HIT2; ++i) {
         int rand = (int)(Math.random() * 3.0D);
         if(rand == 1) {
            this.screen.setColor(new Color(100, 100, 250));
   ...
   private void updateParticlesHit3() {
      for(int i = 0; i < this.NUM_PARTICLES_HIT3; ++i) {
         int incrY = (int)(Math.random() * 5.0D + 1.0D);
         int incrX = (int)(Math.random() * (double)this.p2XV + 1.0D);
         int randY = (int)(Math.random() * (double)this.p2YV / 2.0D + 1.0D);
         int randX = (int)(Math.random() * 10.0D + 1.0D);
   ...
   private void drawParticlesHit3() {
      for(int i = 0; i < this.NUM_PARTICLES_HIT3; ++i) {
         int rand = (int)(Math.random() * 3.0D);
         if(rand == 1) {
            this.screen.setColor(new Color(100, 100, 250));
   ...
   private void controlP2v3() {
      ...
      if(this.p2TouchingGoal > 0 && 60 - this.p2TouchingGoal < 3 + (this.p2X - 850) / SLIMEVEL) {
         this.p2XV = -SLIMEVEL;
      }
   ...
   private void method_rn_WaterPoloSlime_p_1(String s) {
